        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 pt-8">
                {stats.map((stat, index) => (
                  <div key={index} className="text-center group">
                    <div className="text-xl sm:text-2xl text-primary group-hover:scale-110 transition-transform duration-300">{stat.number}</div>
            ...
            <div className="relative flex justify-center lg:justify-end w-full max-w-sm sm:max-w-md lg:max-w-lg xl:max-w-xl mx-auto lg:mx-0">
              {/* Orbital Ring Animations - Responsive Sizes */}
              ...
              {/* Floating Circular Elements - Responsive Positioning */}
              ...
              {/* Pulsing Background Circles - Responsive */}
              ...
              {/* Main Logo Container - Circular & Responsive */}
              <div className="relative bg-gradient-to-br from-white via-white to-accent/10 rounded-full shadow-xl sm:shadow-2xl hover:shadow-2xl sm:hover:shadow-3xl transition-all duration-500 p-6 sm:p-8 md:p-10 lg:p-12 xl:p-16 2xl:p-20 border border-border/20 backdrop-blur-sm group hover:scale-105 aspect-square flex items-center justify-center w-full">
                ...
                <img 
                  ...
                  className="h-32 sm:h-40 md:h-48 lg:h-56 xl:h-64 2xl:h-72 w-auto relative z-10 drop-shadow-lg sm:drop-shadow-2xl group-hover:drop-shadow-xl sm:group-hover:drop-shadow-3xl transition-all duration-500 group-hover:scale-110"
                />
                
                {/* Orbiting particles - Simplified for Mobile */}
                <div className="absolute inset-0 animate-spin hidden sm:block" style={{animationDuration: '8s'}}>
                  <div className="absolute top-3 sm:top-4 left-1/2 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-primary/40 rounded-full transform -translate-x-1/2"></div>
                </div>
                <div className="absolute inset-0 animate-spin hidden sm:block" style={{animationDuration: '12s', animationDirection: 'reverse'}}>
                  <div className="absolute bottom-3 sm:bottom-4 right-1/2 w-1 h-1 sm:w-1.5 sm:h-1.5 bg-accent/50 rounded-full transform translate-x-1/2"></div>
                </div>
                <div className="absolute inset-0 animate-spin hidden md:block" style={{animationDuration: '10s'}}>
                  <div className="absolute top-1/2 right-3 sm:right-4 w-1 h-1 bg-primary/30 rounded-full transform -translate-y-1/2"></div>
                </div>
                <div className="absolute inset-0 animate-spin hidden md:block" style={{animationDuration: '14s', animationDirection: 'reverse'}}>
                  <div className="absolute top-1/2 left-3 sm:left-4 w-1 h-1 bg-accent/40 rounded-full transform -translate-y-1/2"></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-20 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center space-y-4 mb-16">
            <h2 className="text-3xl lg:text-4xl">I nostri servizi</h2>
